...
#ifndef MPI_FILE_GET_POSITION_WORKAROUND
      MPI_Barrier(other.m_current->comm);
#endif
  ...
  actualize() const {
    ...
    if (!m_latent) {
      ...
      if (m_latent->comm != MPI_COMM_NULL) {
        MPI_Comm_dup(m_latent->comm, &comm);
        set_throw_exception_errhandler(comm);
      } else {
        comm = MPI_COMM_NULL;
      }
      if (m_latent->info != MPI_INFO_NULL)
        MPI_Info_dup(m_latent->info, &info);
      else
        info = MPI_INFO_NULL;
      if (m_latent->file != MPI_FILE_NULL) {
        MPI_File_open(comm, m_path->c_str(), MPI_MODE_RDONLY, info, &file);
        ...
        if (m_disp) {
          ...
          MPI_File_set_view(
            ...
            *m_filetype,
            m_datarep->data(),
            MPI_INFO_NULL);
          MPI_File_seek(file, m_offset, MPI_SEEK_SET);
        }
      } else {
        file = MPI_FILE_NULL;
      }
